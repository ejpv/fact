...
public class DetalleFacturaClienteImp implements IDetalleFacturaCliente {
    ...
    public int insertar(DetalleFacturaCliente detalleFacturaCliente) {
        ...
        String sql = "INSERT INTO public.detalle_factura_cliente(\n"
                + "            codigo_detalle_cliente, producto, cantidad, precio, total, factura)\n"
                + "    VALUES (?, ?, ?, ?, ?, \n"
                + "            ?);";
        ...
        lstPar.add(new Parametro(5, detalleFacturaCliente.getTotal()));
        lstPar.add(new Parametro(6, detalleFacturaCliente.getFactura().getNumero()));
    ...
    public int modificar(DetalleFacturaCliente detalleFacturaCliente) {
        ...
        String sql = "UPDATE public.detalle_factura_cliente\n"
                + "   SET  producto=?, cantidad=?, precio=?, total=?, factura=?\n"
        ...
        lstPar.add(new Parametro(4, detalleFacturaCliente.getTotal()));
        lstPar.add(new Parametro(5, detalleFacturaCliente.getFactura().getNumero()));
        lstPar.add(new Parametro(6, detalleFacturaCliente.getCodigo()));
        Conexion con = new Conexion();
        try {
    ...
    public DetalleFacturaCliente obtener(int id) throws Exception {
        ...
        String sql = "SELECT codigo_detalle_cliente, producto, cantidad, precio, total, factura\n"
        ...
        try {
            ...
            while (rst.next()) {
                ...
                fac.setTotal(rst.getDouble(5));
                fac.setFactura(sqlFC.obtener(rst.getInt(6)));
            }
        } catch (Exception e) {
            throw e;
        } finally {
